...
The same constant-sign calculation gives the checked companion relation
\[
 b_k=(-1)^{k-1}|b_k|,
 \qquad (-1)^{k-1}b_k=|b_k|\ge0.
 \tag{5}
\]
This is non-strict: strict positivity requires a separate positive-volume
argument for at least one NBC region.

\section{Relation to prior work}

The finite identity (1) belongs to the classical broken-circuit and
chromatic-polynomial framework.  Whitney's broken-circuit expansion, Tutte's
spanning-tree formulation, and Brylawski's broken-circuit-complex treatment
give the relevant discrete background.  The connected-graph expansion of the
Mayer coefficient is standard statistical-mechanical background going back to
Mayer, with rigorous convergence results by Penrose and Ruelle.
...
The contribution is the exact identity (4) for the continuous hard-sphere
Mayer integral, together with its kernel-checked Lean formalization.  The finite
broken-circuit cancellation, the connected Mayer expansion, and the
product-to-flat coordinate change are not claimed as new in isolation.  They
are classical or standard ingredients, explicitly separated from the theorem's
central bridge.

It is incomplete to describe the result as merely a direct specialization and
...
pointwise while the active graph changes with the configuration.  The proof
then turns the pointwise NBC count into measurable tree-owned regions and an
exact finite sum of their volumes.  This is an exact identity, not an unsigned
majorant or a convergence bound.

The coordinate transformation is deliberately not presented as a novel
contribution.  It supplies a fidelity bridge: the theorem is first proved on
the anchored product-Lebesgue space and then transported explicitly to the
usual flat Euclidean presentation.  The mathematical significance is that the
Mayer cancellation is retained as a nonnegative geometric decomposition rather
than discarded through absolute values.  The formal significance is that the
full discrete-to-continuous-to-measure chain is checked by the Lean kernel.

The recorded prior-work search did not identify an exact prior statement of
this continuous hard-sphere NBC-region volume identity.  This is a bounded
search result rather than an absolute priority claim.  The comparison is
therefore precise: classical finite NBC/Tutte and Mayer/tree-graph results are
background ingredients, while the submitted statement supplies the
configuration-dependent pointwise-to-measurable-volume bridge.  No claim of
strict positivity, convergence, or phase-transition consequences is made.
